...
if ($keysearch == "") {
    $query = "select * from donhang WHERE `tinhTrang` = 'Đã giao' LIMIT $start, $records_per_page";
} else {
    // tìm kiếm theo tên nhà cung cấp, nếu muốn tmf kiếm theo địa chỉ cần thêm " or diaChi like '%E$keysearch%'" vào trước limit
    $query = "select * from donhang where maDonhang like '%$keysearch%' AND `tinhTrang` = 'Đã giao' LIMIT $start, $records_per_page";
}
$html = '';
